...
{
  int Rows[131];
  //how many rows each thread is going to be doing
  int nrows;
  int rowIndex;    // tells thread what row to start working on
  scene_t scene;
}mystruct;
...
void *worker(void *thread)
{
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
	...
	: pixel_dx;
	  ...
	  mystruct *info = (mystruct*) thread;

	  for( int px=info->rowIndex; px<info->rowIndex + info->nrows; ++px )
	    {
	      ...
	      for( int py=0; py<height; ++py )
		{
		  ...
		  for( int xs=-halfSamples; xs<=halfSamples; ++xs )
		      ...
		      for( int ys=-halfSamples; ys<=halfSamples; ++ys )
			{
			  ...
			  trace( &(info->scene), sample_color, &pixel_ray, 0 );
...
int main( int argc, char **argv )
{
  ...
  mystruct my_thread_infos[nthreads];
  pthread_t arrayOfThreads[nthreads];
  int i = 0;
  int val;

  //create threads
  int partition = 131 / nthreads;
  int remainder = 131 % nthreads;
  ...
  if (remainder != 0)
    {
      int num = 0;
      for(num = 0; num < nthreads-1; num++)  //loop thru everything except last thread
	{
	  my_thread_infos[num].nrows = partition;
	  my_thread_infos[num].rowIndex = counter;
	  my_thread_infos[num].scene = scene;
	  counter += partition;
	}
      my_thread_infos[nthreads-1].nrows = partition + remainder;
      my_thread_infos[nthreads-1].rowIndex = counter;
      my_thread_infos[nthreads-1].scene = scene;
    }
  else
    {
      int iter = 0;
	...
	{
	  my_thread_infos[iter].nrows = partition;
	  my_thread_infos[iter].rowIndex = counter;
	  my_thread_infos[iter].scene = scene;
	  counter += partition;
	}
    }
  ...
  for(i=0; i < nthreads; i++)
    {
      ...
      val = pthread_create(&arrayOfThreads[i], NULL, worker, &my_thread_infos[i]);
      if (val != 0)
	{
  ...
  for (i=0; i < nthreads; i++)
    {
      val = pthread_join(arrayOfThreads[i], NULL);
      if(val != 0)
	{
